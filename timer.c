...
// Global tick counter for Bresenham Algorithm (imperfect single periods with zero cumulative error)
// This assumes 1 MHz clock speed with 4 MHz XTAL, saves external oscillators and configuration
// Idea from https://www.romanblack.com/one_sec.htm (License is public domain)
// Optimized version (constants divided by 2^6) can use uint16
uint16_t ticks = 0;
...
void __interrupt() TMR1Int(void)
{
	...
	if(ticks >= 15625) {
		ticks -= 15625;	// Subtract one second (1.000.000 Ticks / 64 = 15.625), this retains relative error
        seconds++;
		trigger_conversion = true; // trigger ADC conversion and output in next loop, keeps interrupt servicing short
	}
    return;
}
...
void init_timer(void) {
    ...
    RCONbits.IPEN = 1; // Do not use PIC16 compatibility mode, also enables interrupt system priority feature
