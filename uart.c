...
// Independent initalization, enough for simulated UART output console according to Microchip documentation
void init_uart(void) {
...
void init_uart_baud(uint32_t baudrate) {
    uint16_t reg = (uint16_t) _XTAL_FREQ/(4*(baudrate+1));  // Formula from datasheet: F-OSC / (4*(n+1))
